  ...
  const caseStudies = {
    "coffee-environmental-documentation": {
      ...
      metrics: [
        ...
        { icon: TrendingUp, value: "0", label: "Риск штрафов", color: "text-soft-blue" },
        { icon: Users, value: "1", label: "Довольный клиент", color: "text-sandy-beige" },
        { icon: Leaf, value: "100%", label: "Переработка отходов", color: "text-sea-green" }
      ],
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {caseStudy.images.map((image: string, index: number) => (
                <motion.div
                  key={index}
                  className="glassmorphic rounded-2xl p-4 hover:scale-105 transition-transform duration-500"
                  initial={{ opacity: 0, y: 20 }}
